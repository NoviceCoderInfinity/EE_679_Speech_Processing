...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python check_snr.py <path_to_wav>")
        sys.exit(1)

    acc_signal_energy = 0
    acc_noise_energy = 0

    # sf.blocks reads the file in chunks to save memory
    for block in sf.blocks(sys.argv[1], blocksize=160000):
        # If stereo, take only one channel (WADA is mono-based)
